...
Status: the original three-path meter was deployed in September 2026. The September 15 extension below is locally tested but requires a separate Cloudflare deployment. GitHub Pages remains the origin. Publishing this repository does not deploy the Worker.

## Deploy the September 15 extension

The allowlist and routes now include the checker, tests, quickstart, and two example JSON files, preserving the original three routes and existing D1 schema. The config uses the existing database ID from the deployment ledger. No database creation or migration is needed.

From your authenticated checkout, preserve local configuration before pulling. If the pull conflicts, resolve it without replacing your existing configuration blindly. Then run:
...
Use Node 22.13+ for the meter tests (they use node:sqlite). After deployment, fetch the five added paths with `User-Agent: EPL-Measurement-Probe/1.0`. Confirm their original response bodies and media types, `X-EPL-Meter: v1`, and increments in the existing D1 database. A header alone does not prove storage. The trial notes are in `ops/checker-trial.md`.

For a printed daily report, use `wrangler d1 execute epl-resource-metrics --remote --command "$(cat daily-report.sql)"`. Some Wrangler versions treat `--file` as an import and display execution statistics without the SELECT rows.

The Cloudflare browser beacon cannot measure raw Markdown/JSON fetching. This Worker adds hourly request counters to an exact allowlist using a private D1 table. No public dashboard or client callback is required.
