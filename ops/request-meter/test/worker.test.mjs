...
test('all measured paths preserve the resource body and headers', async () => {
...
test('deployed routes cover every measured asset and JS MIME types remain distinct', async () => {
  const config = JSON.parse(readFileSync(new URL('../wrangler.jsonc', import.meta.url), 'utf8'));
  for (const path of PATHS.keys()) {
    assert(config.routes.some(r => r.pattern === 'escalationpathlabs.com' + path + '*'), path);
  }
  for (const path of ['/resources/moltbook/reconcile.mjs', '/resources/moltbook/reconcile.test.mjs']) {
    for (const type of ['application/javascript', 'text/javascript', 'text/html']) {
      const h = harness(async () => new Response('example', { headers: { 'Content-Type': type } }));
      await h.request(path);
      assert.equal(h.rows()[0].representation, type === 'text/html' ? 'unexpected_type' : 'expected_type');
      h.db.close();
    }
  }
});
